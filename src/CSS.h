void append_page_header() {
  ...
  webpage += F("<head>");
  webpage += F("<title>PH SENSOR WEBPAGE</title>"); // NOTE: 1em = 16px
...
void append_page_footer(){ // Saves repeating many lines of code for HTML page footers
  webpage += F("<ul>");
  webpage += F("<li{text-align:center}><a href='/'>Home</a></li>"); // Lower Menu bar command entries
  webpage += F("</ul>");
  webpage += "<footer>PH SENSOR";
  webpage += F("</body></html>");
}
